...
abstract class BaseFragment<VB : ViewBinding, VM : ViewModel> : Fragment() {
    ...
    protected fun setToolbarActivity(title: String, titleColor: Int? = null, backgroundColor: Int? = null) {
        (requireActivity() as? BaseActivity<*>)?.setToolbarTitle(title)
        (requireActivity() as? BaseActivity<*>)?.setToolbarStyle(titleColor, backgroundColor)
    }
